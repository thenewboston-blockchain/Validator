...
from v1.cache_tools.valid_confirmation_blocks import (
...
class Command(InitializeNode):
    ...
    def add_arguments(self, parser: CommandParser):
        ...
        parser.add_argument(
            "--head_block_hash", type=str_length_validator(length=HEAD_HASH_LENGTH)
        )
    ...
    def get_head_block_hash(self, value=None):
        ...
        while not valid:
            ...
            if len(head_block_hash) != HEAD_HASH_LENGTH:
                self._error(
                    f"head_block_hash must be {HEAD_HASH_LENGTH} characters long"
                )
                continue
    ...
    def get_node_type(self, value=None):
        ...
        while not valid:
            ...
            if node_type not in [CONFIRMATION_VALIDATOR, PRIMARY_VALIDATOR]:
                self._error(
                    f"node_type must be one of {CONFIRMATION_VALIDATOR} or {PRIMARY_VALIDATOR}"
                )
                continue
    ...
    def initialize_validator(self):
        ...
        # Create SelfConfiguration and related Validator objects
        SelfConfiguration.objects.create(**self.required_input, node_type=node_type)
        sync_accounts_table_to_root_account_file()
